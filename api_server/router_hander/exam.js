  ...
  db.query(sql, (err, results) => {
    ...
    db.query(sql, [req.body.username, req.body.exam_id], (err, results) => {
      if (results.length === 0) {
        ...
        for (
          var i = 0;
          i <= [...Object.entries(req.body.values)].length - 1;
          i++
        ) {
          for (var j = 0; j <= 0; j++) {
        ...
        new Promise((resolve, reject) => {
          let num = [...Object.entries(req.body.values)].length;
          var arr = new Array();
          for (
            var i = 0;
            i <= [...Object.entries(req.body.values)].length - 1;
  ...
  db.query(sql, [req.body.username, req.body.exam_id], (err, results) => {
    ...
    for (let i = 0; i < results.length; i++) {
      if (results[i].question_answer === results[i].answer) {
        ...
        db.query(
          sql1,
          [results[i].username, results[i].answer_id, results[i].exam_id],
          (err, result) => {
            if (result.length === 0) {
...
exports.addquestion = (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    db.query(
      sql,
      [examInfo.exam_type, examInfo.values.title, examInfo.exam_type_classify],
      (err, results) => {
        if (err) return res.cc(err);
            ...
            for (let i = 0; i < examInfo.values.exam.length; i++) {
              ...
              db.query(
                ...
                [
                  results[0].exam_id,
                  a,
                  examInfo.values.exam[i].question,
                  examInfo.values.exam[i].A,
                  examInfo.values.exam[i].B,
                  examInfo.values.exam[i].C,
                  examInfo.values.exam[i].D,
                  examInfo.values.exam[i].answer,
                ],
                (err, results) => {
                  if (err) return res.cc(err);
                }
